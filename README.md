...
更清晰的设计应该是这样：
  ...
  | 能力 | normal | customer_service | support | risk_operator | admin | sys_admin |
  |---|---:|---:|---:|---:|---:|---:|
  | 自己资产/充值/提现 | 有 | 有 | 有 | 有 | 有 | 有 |
  | 查看风控后台 | 无 | 有 | 有 | 有 | 有 | 有 |
  | 处理人工审核 | 无 | 有 | 有 | 有 | 有 | 有 |
  | 查看风险地址 | 无 | 有 | 有 | 有 | 有 | 有 |
  | 新增/停用风险地址 | 无 | 可选 | 可选 | 有 | 有 | 有 |
  | 修改提现限额规则 | 无 | 无 | 无 | 有 | 有 | 有 |
  | 查看账号列表 | 无 | 无 | 无 | 无 | 有 | 有 |
  | 修改业务角色 | 无 | 无 | 无 | 无 | 有 | 有 |
  | 设置/降级管理员 | 无 | 无 | 无 | 无 | 无或有限 | 有 |
  | 系统级最高权限 | 无 | 无 | 无 | 无 | 无 | 有 |

  也就是说，当前项目已经有角色体系和入口控制，但 customer_service、support、risk_operator 的权限边界还比较粗。如果你要上线，我建议再做一轮“能力级权限”拆分，比如用 canReviewWithdraw、canManageRiskRules、
  canManageAddressRisks、canViewUsers、canManageUserTypes 这种能力函数替代现在的粗粒度判断。



  | user_type | 中文定位 | 普通资产功能 | 风控后台入口 | 人工审核 | 风险规则/风险地址 | 账号权限管理 | 查看他人钱包/提现记录类接口 |
  |---|---|---:|---:|---:|---:|---:|---:|
  | normal | 普通用户 | 有 | 无 | 无 | 无 | 无 | 无 |
  | customer_service | 客服 | 有 | 有 | 有 | 有 | 无 | 有 |
  | support | 支持人员 | 有 | 有 | 有 | 有 | 无 | 有 |
  | risk_operator | 风控运营 | 有 | 有 | 有 | 有 | 无 | 有 |
  | admin | 管理员 | 有 | 有 | 有 | 有 | 有 | 有 |
  | sys_admin | 系统管理员 | 有 | 有 | 有 | 有 | 有 | 有 |